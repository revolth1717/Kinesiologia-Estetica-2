...
const CONTENT_API_URL =
  ...
  API_URL;
const AUTH_API_URL =
  process.env.NEXT_PUBLIC_XANO_AUTH_API || API_URL;
  ...
  try {
    ...
    for (let idx = 0; idx < items.length; idx++) {
      ...
      const orderTargets = [
        ...
        `${API_URL}${ORDERS_PATH}`,
        `${AUTH_API_URL ?? API_URL}${ORDERS_PATH}`,
        `${CONTENT_API_URL}/orders`,
        `${API_URL}/orders`,
        `${AUTH_API_URL ?? API_URL}/orders`,
        `${CONTENT_API_URL}${ORDERS_PATH}/create`,
        `${API_URL}${ORDERS_PATH}/create`,
        `${AUTH_API_URL ?? API_URL}${ORDERS_PATH}/create`,
      ];
      ...
      for (const target of orderTargets) {
        ...
        if (orderRes && orderRes.status !== 404) break;
      }
      if (!orderRes) {
        results.push({
          order_ok: false,
          order_status: 502,
          order_data: { message: "No order endpoint" },
        });
        await sleep(1200);
        continue;
      }
